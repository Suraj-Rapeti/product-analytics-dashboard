...
const formatChartDate = (value, granularity = 'day') => {
  if (!value) return '';
  if (granularity === 'month' && typeof value === 'string' && value.length <= 4) {
    return value;
  }
...
const formatTooltipDate = (value, granularity = 'day') => {
  if (!value) return '';
  if (granularity === 'month' && typeof value === 'string' && value.length <= 4) {
    return value;
  }
...
const CustomTooltip = ({ active, payload, label, granularity }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg shadow-card-hover px-3 py-2">
      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">{formatTooltipDate(label, granularity)}</p>
...
function RevenueLineChart({
  ...
  subtitle = 'Daily revenue trend - last 30 days',
}) {
  const { isDark } = useTheme();
  const gridColor  = isDark ? '#1E293B' : '#F1F5F9';
  const tickColor  = isDark ? '#475569' : '#94A3B8';
  const xKey = data.length > 0 && Object.prototype.hasOwnProperty.call(data[0], 'name') ? 'name' : 'date';
